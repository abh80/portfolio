...
export default function ViewItem({children}: { children: JSX.Element }) {
    ...
    return (
        <>
            <header className={"sticky top-0 w-full z-40 md:w-3/4 mx-auto p-2 backdrop-blur transition-colors border-slate-50/[0.05] border-b-2 " + styles.mainHeader}>
                <div

                    className="p-2 w-full  bg-[#00000000]">
                    <Link href={"/"} className={"flex gap-2 w-fit " + styles.logoH}>
                        <Image src={"/adaptive-logo-dark.svg"} alt={"Main logo for header"} width={30} height={30}/>
                        <h1 className={roboto.className + " text-white text-2xl"}>Abh80</h1>
                    </Link>
                </div>
            </header>

            <div id={"main-content"} className={"md:w-3/4 w-full mx-auto p-2 " + styles.mainContent}>
